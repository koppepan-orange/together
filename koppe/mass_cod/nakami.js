...
/*
撫で（stroking）検知ロジック：
- document の pointermove を監視して、現在座標が頭要素内にあるときのみサンプルを蓄える
- 一定時間内（WINDOW_MS）における「有意な横移動」の方向変化回数を数える
- 方向変化回数 >= REQUIRED_DIR_CHANGES で撫で判定（クールダウンあり）
調整しやすくコメント多めにしておいた。閾値いじれ。
*/
