...
let uppC = {
    impD: uppD.querySelector('.import'),
    savD: uppD.querySelector('.save'),
    namD: uppD.querySelector('.name'),
}
uppC.savD.addEventListener('click', () => {
    nanika();
})
async function nanika(){
    let name = uppC.namD.value.trim();
    if(!name) {
        alert("名前を入力してくれ");
        return;
    }
    ...
    await writable.write(JSON.stringify(meiboData, null, 2));
    await writable.close();

    console.log(`${name}.json 保存完了`);
}